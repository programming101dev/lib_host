...
static void write_outcome(const char *wrapper, const char *domain, const char *symbol, int code, int passed)
{
    ...
    {
        written = fprintf(outcome_stream, "P101WRAPPER\t1\tFAULT\t%s\tlib_host\t%s\t%s\t%s\t%d\t%s\n", P101_TEST_PLATFORM, wrapper, domain, symbol, code, passed ? "PASS" : "FAIL");
...
static void test_p101_confstr(struct p101_env *env, struct p101_error *err)
{
    ...
    static const char *const error_names[] = {"EINVAL", "EIO", "ENOMEM"};
#elif defined(__FreeBSD__)
    static const int         errors[]      = {EINVAL};
    static const char *const error_names[] = {"EINVAL"};
#else
    ...
    for(size_t index = 0U; index < sizeof(errors) / sizeof(errors[0]); index++)
    {
        ...
        size_t result = p101_confstr(env, err, 0, argument_3, 0);
        (void)result;
        ...
        write_outcome("p101_confstr", "errno", error_names[index], state.code, failures == failures_before);
    ...
    p101_env_set_fault_injector(env, NULL, NULL);
    {
        ...
        if(native_pid == 0)
        {
            ...
            size_t native_result               = p101_confstr(native_env, native_err, 0, native_argument_3, 0);
            (void)native_result;
            if(p101_error_has_error(native_err))
            {
                fprintf(stderr, "native smoke failed: p101_confstr: %s\n", p101_error_get_message(native_err));
                native_passed = false;
            }
        ...
        if(native_pid > 0)
        {
            ...
            {
                fprintf(stderr, "native smoke terminated by signal: p101_confstr: %d\n", WTERMSIG(native_status));
            }
            ...
            if(WIFEXITED(native_status))
            {
                ...
                {
                    fprintf(stderr, "native smoke exited unsuccessfully: p101_confstr: %d\n", WEXITSTATUS(native_status));
                }
...
static void test_p101_getdomainname(struct p101_env *env, struct p101_error *err)
{
    ...
    for(size_t index = 0U; index < sizeof(errors) / sizeof(errors[0]); index++)
    {
        ...
        errno                 = P101_TEST_ERRNO_SENTINEL;
        p101_env_set_fault_injector(env, fail_next_call, &state);
        int result = p101_getdomainname(env, err, argument_2, 0);
        (void)result;
        EXPECT(state.checks == 1);
        EXPECT(p101_error_is_errno(err, state.code));
        EXPECT(errno == P101_TEST_ERRNO_SENTINEL);
        ...
        EXPECT(fault_resource_events == 0U);
        write_outcome("p101_getdomainname", "errno", error_names[index], state.code, failures == failures_before);
        p101_error_reset(err);
    }
    p101_env_set_fault_injector(env, NULL, NULL);
    {
        ...
        if(native_pid == 0)
        {
            ...
            int  native_result               = p101_getdomainname(native_env, native_err, native_argument_2, 0);
            (void)native_result;
            if(p101_error_has_error(native_err))
            {
                fprintf(stderr, "native smoke failed: p101_getdomainname: %s\n", p101_error_get_message(native_err));
                native_passed = false;
            }
            native_child_status = native_passed ? EXIT_SUCCESS : EXIT_FAILURE;
        native_child_done_:
        ...
        if(native_pid > 0)
        {
            ...
            if(WIFSIGNALED(native_status))
            {
                fprintf(stderr, "native smoke terminated by signal: p101_getdomainname: %d\n", WTERMSIG(native_status));
            }
            EXPECT(WIFEXITED(native_status));
            if(WIFEXITED(native_status))
            {
                if(WEXITSTATUS(native_status) != EXIT_SUCCESS)
                {
                    fprintf(stderr, "native smoke exited unsuccessfully: p101_getdomainname: %d\n", WEXITSTATUS(native_status));
                }
                EXPECT(WEXITSTATUS(native_status) == EXIT_SUCCESS);
            }
        }
...
static void test_p101_gethostname(struct p101_env *env, struct p101_error *err)
{
    ...
    static const char *const error_names[] = {"EFAULT", "ENAMETOOLONG"};
#elif defined(__APPLE__)
    static const int         errors[]      = {EFAULT, ENAMETOOLONG, EPERM};
    static const char *const error_names[] = {"EFAULT", "ENAMETOOLONG", "EPERM"};
#elif defined(__FreeBSD__)
    static const int         errors[]      = {EFAULT, ENAMETOOLONG, EPERM};
    static const char *const error_names[] = {"EFAULT", "ENAMETOOLONG", "EPERM"};
#else
    ...
    for(size_t index = 0U; index < sizeof(errors) / sizeof(errors[0]); index++)
    {
        ...
        errno                 = P101_TEST_ERRNO_SENTINEL;
        p101_env_set_fault_injector(env, fail_next_call, &state);
        int result = p101_gethostname(env, err, argument_2, 0);
        (void)result;
        EXPECT(state.checks == 1);
        EXPECT(p101_error_is_errno(err, state.code));
        EXPECT(errno == P101_TEST_ERRNO_SENTINEL);
        ...
        EXPECT(memcmp(argument_2, argument_2_before, sizeof(argument_2)) == 0);
        EXPECT(fault_resource_events == 0U);
        write_outcome("p101_gethostname", "errno", error_names[index], state.code, failures == failures_before);
        p101_error_reset(err);
    }
    p101_env_set_fault_injector(env, NULL, NULL);
    {
        ...
        if(native_pid == 0)
        {
            ...
            char native_argument_2[PATH_MAX] = {0};
            int  native_result               = p101_gethostname(native_env, native_err, native_argument_2, 0);
            (void)native_result;
            if(p101_error_has_error(native_err))
            {
                fprintf(stderr, "native smoke failed: p101_gethostname: %s\n", p101_error_get_message(native_err));
                native_passed = false;
            }
            native_child_status = native_passed ? EXIT_SUCCESS : EXIT_FAILURE;
        native_child_done_:
        ...
        if(native_pid > 0)
        {
            ...
            if(WIFSIGNALED(native_status))
            {
                fprintf(stderr, "native smoke terminated by signal: p101_gethostname: %d\n", WTERMSIG(native_status));
            }
            EXPECT(WIFEXITED(native_status));
            if(WIFEXITED(native_status))
            {
                if(WEXITSTATUS(native_status) != EXIT_SUCCESS)
                {
                    fprintf(stderr, "native smoke exited unsuccessfully: p101_gethostname: %d\n", WEXITSTATUS(native_status));
                }
                EXPECT(WEXITSTATUS(native_status) == EXIT_SUCCESS);
            }
        }
...
static void test_p101_getloadavg(struct p101_env *env, struct p101_error *err)
{
    ...
    for(size_t index = 0U; index < sizeof(errors) / sizeof(errors[0]); index++)
    {
        ...
        errno                 = P101_TEST_ERRNO_SENTINEL;
        p101_env_set_fault_injector(env, fail_next_call, &state);
        int result = p101_getloadavg(env, err, argument_2, 0);
        (void)result;
        EXPECT(state.checks == 1);
        EXPECT(p101_error_is_errno(err, state.code));
        EXPECT(errno == P101_TEST_ERRNO_SENTINEL);
        ...
        EXPECT(memcmp(argument_2, argument_2_before, sizeof(argument_2)) == 0);
        EXPECT(fault_resource_events == 0U);
        write_outcome("p101_getloadavg", "errno", error_names[index], state.code, failures == failures_before);
        p101_error_reset(err);
    }
    p101_env_set_fault_injector(env, NULL, NULL);
    {
        ...
        if(native_pid == 0)
        {
            ...
            double native_argument_2 = {0};
            int    native_result     = p101_getloadavg(native_env, native_err, &native_argument_2, 0);
            (void)native_result;
            if(p101_error_has_error(native_err))
            {
                fprintf(stderr, "native smoke failed: p101_getloadavg: %s\n", p101_error_get_message(native_err));
                native_passed = false;
            }
            native_child_status = native_passed ? EXIT_SUCCESS : EXIT_FAILURE;
        native_child_done_:
        ...
        if(native_pid > 0)
        {
            ...
            if(WIFSIGNALED(native_status))
            {
                fprintf(stderr, "native smoke terminated by signal: p101_getloadavg: %d\n", WTERMSIG(native_status));
            }
            EXPECT(WIFEXITED(native_status));
            if(WIFEXITED(native_status))
            {
                if(WEXITSTATUS(native_status) != EXIT_SUCCESS)
                {
                    fprintf(stderr, "native smoke exited unsuccessfully: p101_getloadavg: %d\n", WEXITSTATUS(native_status));
                }
                EXPECT(WEXITSTATUS(native_status) == EXIT_SUCCESS);
            }
        }
...
static void test_p101_setdomainname(struct p101_env *env, struct p101_error *err)
{
    ...
    for(size_t index = 0U; index < sizeof(errors) / sizeof(errors[0]); index++)
    {
        ...
        EXPECT(result == (-1));
        EXPECT(fault_resource_events == 0U);
        write_outcome("p101_setdomainname", "errno", error_names[index], state.code, failures == failures_before);
        p101_error_reset(err);
    }
    p101_env_set_fault_injector(env, NULL, NULL);
    {
        ...
        if(native_pid == 0)
        {
            ...
            int native_result = p101_setdomainname(native_env, native_err, "p101", 0);
            (void)native_result;
            if(p101_error_has_error(native_err))
            {
                fprintf(stderr, "native smoke failed: p101_setdomainname: %s\n", p101_error_get_message(native_err));
                native_passed = false;
            }
            native_child_status = native_passed ? EXIT_SUCCESS : EXIT_FAILURE;
        native_child_done_:
        ...
        if(native_pid > 0)
        {
            ...
            if(WIFSIGNALED(native_status))
            {
                fprintf(stderr, "native smoke terminated by signal: p101_setdomainname: %d\n", WTERMSIG(native_status));
            }
            EXPECT(WIFEXITED(native_status));
            if(WIFEXITED(native_status))
            {
                if(WEXITSTATUS(native_status) != EXIT_SUCCESS)
                {
                    fprintf(stderr, "native smoke exited unsuccessfully: p101_setdomainname: %d\n", WEXITSTATUS(native_status));
                }
                EXPECT(WEXITSTATUS(native_status) == EXIT_SUCCESS);
            }
        }
...
static void test_p101_sysconf(struct p101_env *env, struct p101_error *err)
{
    ...
    for(size_t index = 0U; index < sizeof(errors) / sizeof(errors[0]); index++)
    {
        ...
        EXPECT(result == (-1));
        EXPECT(fault_resource_events == 0U);
        write_outcome("p101_sysconf", "errno", error_names[index], state.code, failures == failures_before);
        p101_error_reset(err);
    }
    p101_env_set_fault_injector(env, NULL, NULL);
    {
        ...
        if(native_pid == 0)
        {
            ...
            long native_result = p101_sysconf(native_env, native_err, 0);
            (void)native_result;
            if(p101_error_has_error(native_err))
            {
                fprintf(stderr, "native smoke failed: p101_sysconf: %s\n", p101_error_get_message(native_err));
                native_passed = false;
            }
            native_child_status = native_passed ? EXIT_SUCCESS : EXIT_FAILURE;
        native_child_done_:
        ...
        if(native_pid > 0)
        {
            ...
            if(WIFSIGNALED(native_status))
            {
                fprintf(stderr, "native smoke terminated by signal: p101_sysconf: %d\n", WTERMSIG(native_status));
            }
            EXPECT(WIFEXITED(native_status));
            if(WIFEXITED(native_status))
            {
                if(WEXITSTATUS(native_status) != EXIT_SUCCESS)
                {
                    fprintf(stderr, "native smoke exited unsuccessfully: p101_sysconf: %d\n", WEXITSTATUS(native_status));
                }
                EXPECT(WEXITSTATUS(native_status) == EXIT_SUCCESS);
            }
        }
...
static void test_p101_uname(struct p101_env *env, struct p101_error *err)
{
    ...
    for(size_t index = 0U; index < sizeof(errors) / sizeof(errors[0]); index++)
    {
        ...
        EXPECT(result == (-1));
        EXPECT(fault_resource_events == 0U);
        write_outcome("p101_uname", "errno", error_names[index], state.code, failures == failures_before);
        p101_error_reset(err);
    }
    p101_env_set_fault_injector(env, NULL, NULL);
    {
        ...
        if(native_pid == 0)
        {
            ...
            struct utsname native_argument_2 = {0};
            int            native_result     = p101_uname(native_env, native_err, &native_argument_2);
            (void)native_result;
            if(p101_error_has_error(native_err))
            {
                fprintf(stderr, "native smoke failed: p101_uname: %s\n", p101_error_get_message(native_err));
                native_passed = false;
            }
            native_child_status = native_passed ? EXIT_SUCCESS : EXIT_FAILURE;
        native_child_done_:
        ...
        if(native_pid > 0)
        {
            ...
            if(WIFSIGNALED(native_status))
            {
                fprintf(stderr, "native smoke terminated by signal: p101_uname: %d\n", WTERMSIG(native_status));
            }
            EXPECT(WIFEXITED(native_status));
            if(WIFEXITED(native_status))
            {
                if(WEXITSTATUS(native_status) != EXIT_SUCCESS)
                {
                    fprintf(stderr, "native smoke exited unsuccessfully: p101_uname: %d\n", WEXITSTATUS(native_status));
                }
                EXPECT(WEXITSTATUS(native_status) == EXIT_SUCCESS);
            }
        }
